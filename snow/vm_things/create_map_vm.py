...
credentials = ee.ServiceAccountCredentials(service_account, 'ee/johan_service_account.json')
...
def add_ee_layer_showT(self, ee_image_object, vis_params, name):
  ...
  folium.raster_layers.TileLayer(
      ...
      attr = '&copy; <a href="https://earthengine.google.com/">Google Earth Engine</a>, Copernicus Sentinel data 2024',
      name = name,
      overlay = True,
      control = True,
      show = True
...
with open('map_rgb.js', 'w') as f:
    f.write(stringToWrite)

# with open('/datasets/google-cloud-bucket-public/map_rgb.js', 'w') as f:
#     f.write(stringToWrite)
...
def write_to_gcs(data, bucket_name, file_name):
    try:
        # Explicitly specify the project if there are multiple credentials
...
write_to_gcs(stringToWrite, 'johan_public', 'map_rgb.js')